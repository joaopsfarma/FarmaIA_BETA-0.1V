...
import { Upload, FileText, CheckCircle, ShoppingCart, Download, Search, AlertCircle, Package, ArrowRight, Layers, TrendingUp, Calculator, FileDown } from 'lucide-react';
...
const parseCSV = (text: string) => {
  ...
  if (curField || text[text.length - 1] === ',') curRow.push(curField);
...
export const TransferRequest: React.FC = () => {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'all' | 'medicamento' | 'material'>('all');
  ...
  const processData = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const finalInventory = Object.values(productsMap).map(p => {
        ...
        let suggestion = targetStock - p.stock;
      ...
      // Ordenar por produtos que precisam ser comprados primeiro
      finalInventory.sort((a, b) => b.suggested - a.suggested);
  ...
  const filteredInventory = useMemo(() => {
    return inventory.filter(item => {
      ...
      const isMed = isComprimido || isFrasco || isBolsa || medKeywords.some(k => name.includes(k) || unit.includes(k));
      ...
      const matKeywords = [
        ...
        "DRENO", "ELETRODO", "FITA", "PAPEL", "COLETOR", "ALGODAO", "GAZ", "FIXADOR"
      ];
      const isMat = matKeywords.some(k => name.includes(k) || unit.includes(k)) || (!isMed && !isComprimido && !isFrasco && !isBolsa);

      if (categoryFilter === 'medicamento') {
        if (isMat && !isMed) return false; // Se for material e não tiver cara de med, esconde
